...
export const treatmentCost=(p:Player,id:Ailment,clinic:boolean)=>Math.ceil((ailments[id].cost+(clinic?0:15))*(clinic&&p.body.commission==='done'?.8:1));
export function clinicGreeting(g:Game){
 const p=g.player;
 if(p.body.commission==='done')return 'Dr. Pell: “'+p.name+'. The repair bench is still running thanks to you. Let me see what needs fixing.”';
 if(g.run.recoveries>1)return 'Dr. Pell: “'+p.name+', I know your scars by now. Tell me where it hurts.”';
 if(g.run.recoveries)return 'Dr. Pell: “Good to see you on your feet, '+p.name+'. How is the repair holding?”';
 return 'Dr. Pell: “'+p.name+', was it? I can repair what the streets did, or make you harder to break.”';
}
...
export function medicalCommand(source:Game,verb:string,arg:string):Result|null{
 ...
 if(talking||verb==='implants'){const reply=result([doctor.clinic?clinicGreeting(source):doctor.name+': “I can repair what the streets did, or make you harder to break.”',...bodyReport(p),...(doctor.clinic&&p.body.commission==='done'?['CLINIC / Trusted helper · 20% off injury treatment.']:[]),...p.body.ailments.map(id=>'Treat '+id+' · '+treatmentCost(p,id,doctor.clinic)+' credits.'), instruction('Type treat <condition> or treat all.'),...Object.entries(implants).flatMap(([id,d])=>[id+' · rank '+rank(p,id as Implant)+'/3 · '+d.description+' '+(rank(p,id as Implant)<3?'Next: '+d.costs[rank(p,id as Implant)]+' credits.':'Fully installed.'),...(rank(p,id as Implant)<3?[instruction('Type upgrade '+id+'.')]:[])]),...(p.body.commission==='none'?[doctor.name+': “Bring me three pieces of salvage for the repair bench and I’ll fit an implant for you. No charge.”',instruction('Type accept spare parts.')]:medicalJournal(p))]);reply.sound='npc-reply';return reply;}
 ...
 if(verb==='treat'){
  ...
  const cost=selected.reduce((n,id)=>n+treatmentCost(p,id,doctor.clinic),0);
