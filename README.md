...
The [runs and combat guide](design/RUNS-AND-COMBAT.md) explains clinic reconstruction, packed revival drones, aiming/reload, bleeding, overheat and surrender. [Playtest sign-off](design/PLAYTEST-LEDGER.md) stays separate from automated verification. Six disposable saves let you test the new mechanics without a long journey.

The [medical progression guide](design/MEDICINE.md) covers six lasting conditions, paid treatment, three ranked implant paths, and the Spare Parts commission. At the clinic, type `talk doctor`. HP uses a red gauge, stamina yellow, and radiation a ☢ count. `status` lists lasting effects. Clinic reconstruction retains implants and adds an injury; an explicit New game starts fresh. See [clinic continuity and combat clarity](design/CLINIC-RECOVERY-AND-COMBAT.md).
...
Start with `talk clerk`, `s`, `e`, `talk technician`. `help` lists commands. The command input supports history, completion and log scrolling. Typing and clickable action buttons use the same game rules. Current inhabitants, ground items, enemy artwork and combat actions appear inside the transcript. Hidden theft opportunities remain prose discoveries. Type `map`, `inventory`, `skills`, `journal`, `settings`, or `help` to open the relevant popover or reference panel. At Clinic Steps, type `down` to descend through the manhole into Steps Undercroft, then `up` to return. The map follows the current floor and marks ladders. Settings includes palettes, text size, audio credits, saves and recovery. A stale tab cannot silently overwrite another session: load the latest save, or explicitly recover a living branch. A fallen character returns through Wake at clinic; stale life identities cannot overwrite a completed reconstruction. Conflicting branches can be downloaded from quarantine.
...
The desktop frame is centered, capped at **1440px wide**, and fills the available height. A small room painting sits beside player data; the green-on-black transcript fills the remaining space. Hover or focus the room art for a full-scene preview, which closes on leave/blur/Escape. Type `view` for an explicit artwork dialog. Complete 16:9 paintings remain uncropped. Ultrawide monitors retain dark gutters; narrow screens keep the same compact header. Gameplay has no masthead or permanent sidebar. Mouse buttons and typed commands are both supported. See `design/VISUAL-DIRECTION.md`.
...
**Tab** toggles a stationary minimap popover during exploration; Escape also closes it. It always opens in local close-up, centered on the current room and floor. Fog of war is mandatory: exploration reveals terrain and room markers. The optional world overview respects the same fog. World actions and room activity pause while open. It is unavailable during combat, clinic intake, recovery or other modals. Map shortcuts: L local, W world, S surface, B below, G labels, H hazards. **Ctrl+Space** now completes commands; Tab retains normal focus navigation inside inventory and clinic popovers.
...
`npm run test:medicine # medical progression and hover/focus layout
