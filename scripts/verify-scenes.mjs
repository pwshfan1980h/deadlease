...
try{
 ...
 await cmd('talk doctor');
 const hint=page.locator('.log .transcript-instruction').filter({hasText:'Type accept spare parts.'});assert.equal(await hint.count(),1);assert.equal(await hint.evaluate(e=>getComputedStyle(e).color),'rgb(165, 170, 167)');assert.equal(await hint.evaluate(e=>getComputedStyle(e).fontStyle),'italic');assert.equal(await hint.locator('.item-mention,.character-name').count(),0);
 const offer=page.locator('.log p').filter({hasText:'Bring me three pieces of salvage'});assert(!(await offer.innerText()).includes('Type'));assert.equal(await offer.evaluate(e=>getComputedStyle(e).fontStyle),'normal');assert.equal(await offer.evaluate(e=>getComputedStyle(e).color),'rgb(167, 249, 135)');assert(!(await page.locator('.log').innerText()).includes('HINT /'));
 await shot('doctor-world-and-instructions');checks.push('Doctor dialogue stays in-world and green; separate control hints are grey italic on black, with no item/name color leaking into hints');
 ...
 await cmd('look around');assert.equal(await page.locator('.ground-text').count(),0);assert.equal(await page.locator('.ground-items').count(),1);assert.deepEqual(await page.locator('.ground-items li').allTextContents(),['knife ×2','medical supplies']);assert.equal(await color('.ground-items'),'rgb(177, 180, 178)');await shot('1440-ground-items');await cmd('take knife');assert.equal(await page.locator('.ground-items').innerText(),'medical supplies');await cmd('take all');assert.equal(await page.locator('.ground-items').count(),0);checks.push('One immediate plain grey ground inventory, no repeated list after look, typed pickup updates without stale items');
