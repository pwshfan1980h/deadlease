...
export function Atlas({game,palette="original",request}:{game:Game;palette?:string;request?:{text:string;serial:number}|null}){
 const [layer,setLayer]=useState(rooms[game.room].layer),[labels,setLabels]=useState(true),[hazards,setHazards]=useState(true),[selected,setSelected]=useState(game.room),[zoom,setZoom]=useState(true);
 ...
 useEffect(()=>{
  ...
  if(action==='surface')changeLayer(0);if(action==='sewers')changeLayer(-1);
  if(action==='local')setZoom(true);if(action==='world')setZoom(false);
  const update=(old:boolean)=>value?value==='on':!old;
  if(action==='labels')setLabels(update);if(action==='hazards')setHazards(update);
 ...
 const regions=[['district','DISTRICT 67',1,-.9],['quay','SALT QUAY',8.1,3.2],['foundry','CINDER UNION',6.4,-4.4],['reed','REEDWARD FEN',-1,8.5],['glass','VITREOUS WARD',-3.1,-3.25],['crown','SURVEY CROWN',2,-8.05],['wilds','LONG DIKE',6.5,11.5],['bellwether','BELLWETHER',4.1,16.1]] as [string,string,number,number][];
 ...
 const changeLayer=(next:number)=>{setLayer(next);setZoom(true);if(selectedRoom.layer!==next){const known=game.discovered.filter(id=>rooms[id].layer===next).sort((a,b)=>Math.hypot(rooms[a].x-current.x,rooms[a].y-current.y)-Math.hypot(rooms[b].x-current.x,rooms[b].y-current.y));if(known[0])setSelected(known[0])}};
 ...
 <div className="map-options"><span className="map-fog-status">Fog of war</span><label><input type="checkbox" checked={labels} onChange={e=>setLabels(e.target.checked)}/> Labels</label><label><input type="checkbox" checked={hazards} onChange={e=>setHazards(e.target.checked)}/> Hazards</label></div>
 <svg ref={svg} className="geographic-map" viewBox={extent} role="img" aria-label={`Geographic ${layer===0?'surface':'sewer'} map with terrain, streets, footprints and discovery fog`}>
  <defs><pattern id={'water'+uid} width=".6" height=".4" patternUnits="userSpaceOnUse"><path d="M0 .2h.3" stroke="var(--teal)" strokeWidth=".015" opacity=".35"/></pattern><mask id={mask} maskUnits="userSpaceOnUse" x="-20" y="-20" width="50" height="50"><rect x="-20" y="-20" width="50" height="50" fill="black"/>{game.discovered.filter(id=>rooms[id].layer===layer).map(id=><circle key={id} cx={rooms[id].x} cy={rooms[id].y} r=".83" fill="white"/>)}</mask></defs>
  <rect x="-20" y="-20" width="50" height="50" fill="var(--soot)"/>
  <g className="map-terrain" mask={`url(#${mask})`}>
  {layer===0?<>
  ...
  {geometry.streets.filter(s=>s.layer===layer&&(visible(s.id)||visible(s.to))).map(s=><g key={s.id+s.to}><path d={`M${s.x} ${s.y}L${s.x2} ${s.y2}`} stroke="var(--soot)" strokeWidth=".17"/><path d={`M${s.x} ${s.y}L${s.x2} ${s.y2}`} stroke="var(--ash)" strokeWidth={Math.max(.035,unit*1.2)}/></g>)}
  {geometry.buildings.filter(b=>b.layer===layer&&visible(b.id)).map((b,i)=><rect opacity=".22" key={i} x={b.x} y={b.y} width={b.w} height={b.h} fill="var(--soot)" stroke="var(--ash)" strokeWidth=".018"/>)}
  {geometry.shafts.filter(s=>game.discovered.some(id=>rooms[id].layer===layer&&rooms[id].x===s.x&&rooms[id].y===s.y)).map(s=><g key={s.id} aria-label={layer===0?'Ladder down':'Ladder up'}><title>{layer===0?'Ladder down — type down here':'Ladder up — type up here'}</title><path d={`M${s.x-.38} ${s.y-.12}v.3m.16 -.3v.3m-.16 -.24h.16m-.16 .09h.16m-.16 .09h.16`} fill="none" stroke="var(--amber)" strokeWidth=".045"/></g>)}
  </g>
  ...
  {!local&&labels&&layer===0&&regions.filter(([zone,name,x,y])=>game.discovered.some(id=>rooms[id].zone===zone)&&regionClear(name,x,y)).map(([zone,name,x,y])=><text key={zone} x={x} y={Math.max(frame[1]+unit*14,Math.min(y,frame[1]+frame[3]-unit*8))} textAnchor="middle" className="map-region" fill="var(--ash)" fontSize={unit*10} stroke="var(--soot)" strokeWidth={unit*3} paintOrder="stroke">{name}</text>)}
 ...
 <div className="map-legend"><span className="amber"><Sprite id="icon/you" palette={palette} label="Current location"/> You</span><span className="teal"><Sprite id="icon/refuge" palette={palette} label="Refuge"/> Refuge</span><span className="teal"><Sprite id="icon/shaft" palette={palette} label="Ladder"/> Ladder ↕</span></div>
 ...
 {visible(selected)&&selectedRoom.layer===layer&&['down','up'].filter(d=>selectedRoom.exits[d]).map(d=><p className="map-vertical small" key={d}>{d==='down'?'↓':'↑'} <code>{d}</code> → {visible(selectedRoom.exits[d])?rooms[selectedRoom.exits[d]].name:'Unexplored'}</p>)}</div>
 <p className="muted small">{game.discovered.length} / {Object.keys(rooms).length} surveyed. Unexplored areas stay hidden. Map markers inspect locations.</p></div>;
}
