...
const commonCommands=['status','talk doctor','treat all','implants','upgrade dermal weave','upgrade adrenal regulator','upgrade targeting optic','accept spare parts','report spare parts','spare','accept surrender','depart','missions','jobs','deliver parcel',...Object.keys(deliveryRoutes).map(id=>'accept '+id),'view','map surface','map sewers','map local','map world','map labels on','map labels off','map hazards on','map hazards off','pause','menu','help','look','stats','skills','journal','inventory','map','settings','inspect target','fire','shoot','attack','aim','brace','cover','heal','flee','rest','scrounge','take all','save','load','load auto','export','import','mute','up','down','go north','go south','go east','go west','go up','go down','install component','give key commons','give key syndicate','resolve erase','resolve disclose'];
...
export function App({initialGame,storageDriver}:{initialGame?:Game;storageDriver?:StorageDriver}){
 ...
 useEffect(()=>{if(!atTitle&&!pauseOpen&&!intakePanel&&!inventoryOpen&&!mapOpen)input.current?.focus()},[atTitle,pauseOpen,intakePanel,booted,inventoryOpen,mapOpen,game?.run.status,recovering]);
 ...
 async function run(text:string,resolution?:TimingOutcome):Promise<string[]|undefined>{
  ...
  if(cmd.startsWith('map ')){
   const option=cmd.slice(4);if(/^fog(?: |$)/.test(option)){append(['› '+text,'HINT / Fog of war is always on. Explore rooms to reveal them.']);return}
   const valid=/^(surface|sewers|local|world)$/.test(option)||/^(labels|hazards)( (on|off))?$/.test(option);
   const place=g.discovered.find(id=>id===option||rooms[id].name.toLowerCase()===option);
   append(['› '+text,...(!valid&&!place?['Map: surface, sewers, local, world, labels/hazards on/off, or a discovered place name. Fog of war is always on.']:[])]);
 ...
 async function wakeAtClinic(){
  if(traveling.current||saving||blockedRef.current||!repo.current)return;
  const source=gameRef.current;if(!source||source.run.status!=='dead')return;
  traveling.current=true;setSaving(true);
  try{
   await queue.current;const result=await repo.current.reconstruct(source);
   clearInteractions();resetActivity();commit(result.state,false);setRecovering(false);setIntakePanel(null);setInventoryOpen(false);setMapOpen(false);setPanelOpen(false);
   follow.current=false;manualScroll.current=false;resetTranscriptScroll.current=true;setLogs(result.messages);setNotice('');audio.current?.play('npc-reply');audio.current?.play('ambience');
  }catch(e){error(e)}finally{traveling.current=false;setSaving(false);input.current?.focus({preventScroll:true})}
 }
 function deathExperience(before:Game,after:Game){
  if(before.run.status==='dead'||after.run.status!=='dead')return;clearInteractions();resetActivity();setNotice('');audio.current?.battle(false);
 ...
 function openMap(){
  ...
  setPanelOpen(false);setPanel('Atlas');setMapRequest(old=>({text:'local',serial:(old?.serial??0)+1}));setMapOpen(true);
 }
 ...
 {atTitle?<main className="title-screen">
  ...
  <div className="title-menu"><h1>FREEBORN<span>.</span></h1><p>They brought you back to work. Find a way to live.</p>
   <nav aria-label="Main menu" onKeyDown={e=>{if(!['ArrowUp','ArrowDown'].includes(e.key))return;e.preventDefault();const options=Array.from(e.currentTarget.querySelectorAll<HTMLButtonElement>('button:not(:disabled)'));const i=options.indexOf(document.activeElement as HTMLButtonElement);options[(i+(e.key==='ArrowDown'?1:-1)+options.length)%options.length]?.focus()}}>
    {game&&<button className="title-action" onClick={()=>{setAtTitle(false);setNotice('');audio.current?.play('ambience');audio.current?.battle(!!game.encounter)}}>{game.run.status==='dead'?'Return to clinic':'Continue'}</button>}
 ...
 {game?.run.status==='dead'&&!atTitle&&!creating&&<RunEnd game={game} notice={notice} busy={saving} blocked={blocked||!repo.current} onRetry={()=>void recover('auto')} onWake={()=>void wakeAtClinic()} onLoad={()=>void load('auto')} onTitle={returnToMenu} onExport={()=>download(encode(game),'freeborn-ended-'+game.run.id+'.json')}/>}
