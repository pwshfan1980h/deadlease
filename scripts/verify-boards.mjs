...
try{
 ...
 assert((await page.locator('.log').innerText()).includes('deliver parcel'));await cmd('deliver parcel');assert.equal((await state()).player.credits,initial.player.credits+14);await cmd('read jobs board');assert((await page.locator('.log').innerText()).includes('accept clinic-run'));await shot('3440-yard-board');
 const paid=await state();await cmd('deliver parcel');assert.deepEqual(await state(),paid);
 await cmd('accept clinic-run');for(const step of ['w','w','w'])await cmd(step);await cmd('deliver parcel');assert.equal((await state()).courier.completed,2);assert.equal((await page.locator('.log').innerText()).split('Need money? Read the jobs board').length-1,1);checks.push('Fresh character discovers work through explicit look around and clerk; keyboard completion; paid local round trip without global help; no duplicate payout or repeated introduction');
