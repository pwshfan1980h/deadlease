...
export function bodyReport(p:Player){return ['CONDITION / '+(p.body.ailments.length?p.body.ailments.map(id=>id+': '+ailments[id].description).join(' '):'No lasting injuries.'),'IMPLANTS / '+(Object.entries(p.body.implants).map(([id,n])=>id+' rank '+n).join(' · ')||'None installed.'),'Lasting conditions need a doctor: treat <condition> or treat all at Reclamation Clinic or Bellwether Gate. Type talk doctor for prices. Rest, supplies and revival drones do not cure them.'];}
export function medicalJournal(p:Player){return p.body.commission==='none'?[]:['PELL / Spare Parts — '+(p.body.commission==='done'?'Completed. '+(p.body.voucher?'One free implant rank available.':'Upgrade redeemed.'):'Bring 3 salvage to a doctor; report spare parts earns one free implant rank.')];}
export function injury(p:Player,id:Ailment,m:string[]){if(!impaired(p,id)){p.body.ailments.push(id);m.push('INJURY / '+id+'. '+ailments[id].description+' Type status for treatment options.')}}
...
export function medicalCommand(source:Game,verb:string,arg:string):Result|null{
 ...
 if(talking||verb==='implants')return result([doctor.name+': “I can repair what the streets did, or make you harder to break.”',...bodyReport(p),...p.body.ailments.map(id=>'Treat '+id+' · '+(ailments[id].cost+(doctor.clinic?0:15))+' credits.'), 'Type treat <condition> or treat all.',...Object.entries(implants).map(([id,d])=>id+' · rank '+rank(p,id as Implant)+'/3 · '+d.description+' '+(rank(p,id as Implant)<3?'Next: '+d.costs[rank(p,id as Implant)]+' credits. Type upgrade '+id+'.':'Fully installed.')),...(p.body.commission==='none'?['“Bring me 3 salvage for the repair bench and I’ll fit one implant rank free. Type accept spare parts.”']:medicalJournal(p))]);
