...
export function App({initialGame,storageDriver}:{initialGame?:Game;storageDriver?:StorageDriver}){
 ...
 function actionButton(text:string,label=text,unavailable=false){return <button key={text} type="button" disabled={unavailable||!!travelFade||pauseOpen||!!timing||!!discovery||mapOpen||inventoryOpen||endingOpen||game?.run.status==='dead'} onClick={()=>{follow.current=true;void run(text)}}>{label}</button>}
 ...
 </section> {game.encounter&&<div className="transcript-combat">  <div className="command-options" aria-label="Combat commands">{['attack','aim','brace','cover','heal','flee'].map(a=>actionButton(a))}</div>
