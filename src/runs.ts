...
export interface Run {id:string;seed:number;status:'alive'|'dead';cause:string;endedTurn:number|null;kills:number;revivals:number}
export const drone='stitch drone';
export function runRecord(seed:number,id='seed-'+seed):Run{return {id,seed,status:'alive',cause:'',endedTurn:null,kills:0,revivals:0}}
...
export function resolveLethal(g:Game,cause:string,messages:string[]):'revived'|'dead'{
 ...
 g.courier.route=null;delete p.inventory['sealed parcel'];for(const key of Object.keys(p.pack.layout))if(key.startsWith('sealed parcel:'))delete p.pack.layout[key];
 messages.push('RUN ENDED / '+p.name+' died to '+cause+'. This life is over. A new patient will wake at the clinic.');return 'dead';
}
